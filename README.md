...
A local-first SAST tool for server-side web apps. Semgrep surfaces candidate sinks, a self-built cross-file call graph reconstructs how a request reaches each one, and an LLM rules on reachability and gives a fix. Java/Spring, Python (Flask, Django, FastAPI), JavaScript/TypeScript (Express, Koa, NestJS), Go (net/http, gorilla/mux, chi, gin, echo), C++, Rust (actix-web, Rocket, axum), C# (ASP.NET Core attribute routing, MVC convention routing, minimal APIs), PHP (Laravel route facades, Symfony/Laravel `#[Route(...)]` attributes and `@Route(...)` PHPDoc annotations), Ruby (Rails routes.rb per-verb calls and the `resources` RESTful macro), and Kotlin (Spring Boot's own annotation routing, Ktor's `get("/x") { ... }` route-building DSL) all have full call-graph reachability tracing.
...
- **Reproducible rules.** `rules/vendor/semgrep-rules` is a locked submodule, curated for server-side Java/Spring, Python, JavaScript/TypeScript, Go, Rust, C#, PHP, Ruby and Kotlin. C#'s, PHP's and Ruby's vendored coverage are all close to Java's own depth and needed no custom-rule gap filled; Kotlin's is thin like Rust's, with one real (if narrow) command-injection rule and no SQL-injection rule at all. Twenty custom rules under `rules/custom` add project-specific Java/XML coverage, eight C++ checks (`CPP001`–`CPP008`), Rust's command-injection/SQL-injection rules, and Kotlin's own SQL-injection rule (the interpolation half matched by `pattern-regex` rather than a structural pattern, after confirming the Python-f-string-style metavariable-in-string syntax does not match a Kotlin string template at all). C++ rules use Semgrep's C++ AST; `.h` files require C++-specific content before their findings are accepted.
...
- 58 hand-labeled candidates in `eval/labels.json`: 19 Java, 9 Python, 7 JavaScript, 8 C++ (`eval/fixtures/cpp_demo`), 3 Rust (`eval/fixtures/rust_demo`), 3 C# (`eval/fixtures/csharp_demo`), 3 PHP (`eval/fixtures/php_demo`), 3 Ruby (`eval/fixtures/ruby_demo`) and 3 Kotlin (`eval/fixtures/kotlin_demo`). C++ command-execution rules retain request reachability; deterministic CPP004–CPP008 findings use the static-verdict path. Rust's, C#'s, PHP's, Ruby's and Kotlin's three-label sets each exercise the same request-reachability path as Python/JS/Go: two sinks reached through a handler and a service-layer helper, one an orphan nothing calls.
