...
- 49 hand-labeled candidates in `eval/labels.json`: 19 Java, 9 Python, 7 JavaScript, 8 C++ (`eval/fixtures/cpp_demo`), 3 Rust (`eval/fixtures/rust_demo`) and 3 C# (`eval/fixtures/csharp_demo`). C++ command-execution rules retain request reachability; deterministic CPP004–CPP008 findings use the static-verdict path. Rust's and C#'s three-label sets both exercise the same request-reachability path as Python/JS/Go: two sinks reached through a handler and a service-layer helper, one an orphan nothing calls.
