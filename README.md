...
- **Reproducible rules.** `rules/vendor/semgrep-rules` is a locked submodule, curated for server-side Java/Spring, Python, JavaScript/TypeScript, Go, Rust, C#, PHP and Ruby. C#'s, PHP's and Ruby's vendored coverage are all close to Java's own depth (PHP's `php/lang/security` and Ruby's `ruby/lang/security`/`ruby/rails/security` both carry real `mode: taint` command- and SQL-injection rules) and needed no custom-rule gap filled for any of the three passes, unlike Rust's near-empty bucket. Nineteen custom rules under `rules/custom` add project-specific Java/XML coverage, eight C++ checks (`CPP001`–`CPP008`), and Rust command-injection/SQL-injection rules the vendored `rust/lang/security` bucket has no equivalent of. C++ rules use Semgrep's C++ AST; `.h` files require C++-specific content before their findings are accepted.
...
- 55 hand-labeled candidates in `eval/labels.json`: 19 Java, 9 Python, 7 JavaScript, 8 C++ (`eval/fixtures/cpp_demo`), 3 Rust (`eval/fixtures/rust_demo`), 3 C# (`eval/fixtures/csharp_demo`), 3 PHP (`eval/fixtures/php_demo`) and 3 Ruby (`eval/fixtures/ruby_demo`). C++ command-execution rules retain request reachability; deterministic CPP004–CPP008 findings use the static-verdict path. Rust's, C#'s, PHP's and Ruby's three-label sets each exercise the same request-reachability path as Python/JS/Go: two sinks reached through a handler and a service-layer helper, one an orphan nothing calls.
