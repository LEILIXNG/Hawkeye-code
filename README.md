...
- 46 hand-labeled candidates in `eval/labels.json`: 19 Java, 9 Python, 7 JavaScript, 8 C++ (`eval/fixtures/cpp_demo`) and 3 Rust (`eval/fixtures/rust_demo`). C++ command-execution rules retain request reachability; deterministic CPP004–CPP008 findings use the static-verdict path. Rust's three labels exercise the same request-reachability path as Python/JS/Go: two sinks reached through an actix-web handler and a helper function, one an orphan nothing calls.
