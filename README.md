...
A local-first SAST tool for server-side web apps. Semgrep surfaces candidate sinks, a self-built cross-file call graph reconstructs how a request reaches each one, and an LLM rules on reachability and gives a fix. Java/Spring, Python (Flask, Django, FastAPI), JavaScript/TypeScript (Express, Koa, NestJS), Go (net/http, gorilla/mux, chi, gin, echo), C++, Rust (actix-web, Rocket, axum), C# (ASP.NET Core attribute routing, MVC convention routing, minimal APIs), PHP (Laravel route facades, Symfony/Laravel `#[Route(...)]` attributes and `@Route(...)` PHPDoc annotations), Ruby (Rails routes.rb per-verb calls and the `resources` RESTful macro), Kotlin (Spring Boot's own annotation routing, Ktor's `get("/x") { ... }` route-building DSL), and Scala (Play Framework's `conf/routes` file, a non-Scala text format linked onto controller methods by its own parser) all have full call-graph reachability tracing.
...
688 unit tests cover the deterministic half — dedup, path handling, context extraction, the call graph for all eleven language groups, the rule set contract, the HTTP API, and the launcher/window lifecycle. No test makes a real LLM call; LLM quality is tracked separately through `eval/labels.json`.

## How it works

- **Cross-file analysis, eleven language groups.** Semgrep OSS taint analysis stops at the method boundary. `scanner/callgraph/` walks the other way — from the sink up through its callers, across files — until it reaches an entry point a request can come in through. Java, Python and JavaScript/TypeScript retain their framework-specific entry recognizers; Go recognises net/http, gorilla/mux, chi, gin and echo route registrations. C++ recognises Crow, Drogon, Oat++ and gRPC entries and uses tree-sitter types, owners, overloads, template calls, local include macros and common function-pointer flows to remove ambiguous edges. Rust recognises actix-web/Rocket's attribute macros (`#[get("/x")]`) and axum/actix's route-registration calls (`.route(path, get(handler))`); ownership comes from the nearest `impl` block rather than a class body, and a type's trait impls accumulate across however many separate `impl Trait for Type` blocks declare them. C# recognises ASP.NET Core's own attribute routing (`[HttpGet("{id}")]`), MVC's convention routing (any public method on a Controller/ControllerBase-derived type), and minimal-API registration (`app.MapGet("/x", handler)`); a `partial class`'s scattered base-list declarations accumulate the same way Rust's trait impls do. PHP recognises Symfony/Laravel's `#[Route(...)]` attribute and the older `@Route(...)` PHPDoc annotation, plus Laravel's `Route::get('/x', $handler)` facade call — a route-registration call whose handler almost always lives in a different file than the registration, so its named-reference resolution searches the whole index rather than just the registering file. Ruby recognises Rails' routes.rb per-verb calls (`get '/x', to: 'ctrl#action'`) and its `resources :name` RESTful macro (up to seven routes, narrowed by `only:`/`except:`), both resolved the same way PHP's Laravel facade is; a public method on an ApplicationController/ActionController::Base subclass is a weaker hint than C#'s otherwise-identical convention-routing tier, since Rails (unlike ASP.NET MVC) does not expose a controller action without a routes.rb entry. Kotlin recognises Spring Boot's own annotations exactly like Java, and Ktor's route-building DSL — a call ending in a trailing lambda block (`get("/x") { ... }`) is itself the handler, with a `route("/prefix") { ... }` block's own path threaded through however many of them nest around the leaf verb call. Scala is the one language here whose call-graph module recognises no entry point on its own at all: Play Framework has no annotation or registration-call convention whatsoever, so a separate module reads `conf/routes` — a fixed-column text format, not Scala syntax — with its own line parser and links each route line onto the Scala method it names, globally across the index, the same "external file, not source, proves the entry point" shape this project's own MyBatis-mapper linking already uses. A mixed-language checkout indexes into one shared graph.
...
- **Reproducible rules.** `rules/vendor/semgrep-rules` is a locked submodule, curated for server-side Java/Spring, Python, JavaScript/TypeScript, Go, Rust, C#, PHP, Ruby, Kotlin and Scala. C#'s, PHP's, Ruby's and Scala's vendored coverage are all close to Java's own depth and needed no custom-rule gap filled; Kotlin's is thin like Rust's, with one real (if narrow) command-injection rule and no SQL-injection rule at all. Twenty custom rules under `rules/custom` add project-specific Java/XML coverage, eight C++ checks (`CPP001`–`CPP008`), Rust's command-injection/SQL-injection rules, and Kotlin's own SQL-injection rule (the interpolation half matched by `pattern-regex` rather than a structural pattern, after confirming the Python-f-string-style metavariable-in-string syntax does not match a Kotlin string template at all). C++ rules use Semgrep's C++ AST; `.h` files require C++-specific content before their findings are accepted.
...
- 61 hand-labeled candidates in `eval/labels.json`: 19 Java, 9 Python, 7 JavaScript, 8 C++ (`eval/fixtures/cpp_demo`), 3 Rust (`eval/fixtures/rust_demo`), 3 C# (`eval/fixtures/csharp_demo`), 3 PHP (`eval/fixtures/php_demo`), 3 Ruby (`eval/fixtures/ruby_demo`), 3 Kotlin (`eval/fixtures/kotlin_demo`) and 3 Scala (`eval/fixtures/scala_demo`). C++ command-execution rules retain request reachability; deterministic CPP004–CPP008 findings use the static-verdict path. Rust's, C#'s, PHP's, Ruby's, Kotlin's and Scala's three-label sets each exercise the same request-reachability path as Python/JS/Go: two sinks reached through a handler and a service-layer helper, one an orphan nothing calls.
